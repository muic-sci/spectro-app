...
/**
 * Full experiment report (fully client-side / static) — every captured strip,
 * every plot, every result, print-friendly. Loads the experiment from IndexedDB
 * (useExperiment), resolves image/crop object URLs at read time, and recomputes
 * the derived science in the browser. Experiment id comes from `?id=`.
 */
...
import { ReportThemeShell, ReportThemeToggle } from "@/components/report/report-theme";
...
function ImageStrip({ src, label }: { src: string; label: string }) {
  return (
    <figure className="flex flex-col gap-1">
      {/* eslint-disable-next-line @next/next/no-img-element -- dynamic blob: object URL */}
...
function Centered({ children }: { children: React.ReactNode }) {
  return (
    <main className="mx-auto flex min-h-dvh max-w-3xl flex-col items-center justify-center gap-4 px-6 py-10 text-t3">
      {children}
    </main>
  );
}
...
function ReportPageInner() {
  ...
  const { experiment, derived, loading, notFound } = useExperiment(id);
  ...
  // Standards with a spectrum, in chart order (deriveAnalysis sorts by concentration).
  ...
  return (
    <ReportThemeShell>
      ...
      <header className="flex flex-col gap-4">
        ...
        <div className="no-print flex flex-wrap items-center gap-3">
          ...
          <Button
            ...
            onClick={() =>
              downloadResultsCsv(
                { name: experiment.name, mode: experiment.mode, lightType: experiment.lightType },
                derived,
              )
            }
          >
          ...
          <Link href={`/experiment?id=${experiment.id}`} className="text-sm text-t3 hover:text-t1">
            ← Back to wizard
          </Link>
          <ReportThemeToggle />
        </div>
      ...
      <Section title="Method — how the spectrum is measured">
        <Explainer>
          ...
          <ol className="flex list-decimal flex-col gap-2 pl-5 marker:text-t4">
            <li>
              <b className="text-t1">Capture.</b> Each photo is taken with focus and exposure{" "}
              <em>locked</em>, so every photo is directly comparable (the same brightness means the
        ...
        <Section title={`2 · ${t.blankLabel}`}>
          ...
          {calibration ? (
            ...
            <div className="flex flex-wrap items-start gap-5">
              {blankImage.croppedUrl && <ImageStrip src={blankImage.croppedUrl} label={`${t.blankShort} (cropped)`} />}
      ...
      {absSeries.length > 0 && lambdaMax != null && (
        <Section title={`3 · ${t.signal} spectra`}>
          <Explainer>
            ...
            computed{" "}
            {isFluor ? <Formula>F = I − I₀</Formula> : <Formula>A = −log₁₀(I / I₀)</Formula>}{" "}
        ...
        <Section title="4 · Unknown samples">
          ...
          {derived.unknowns.map((u, i) => (
            <div key={u.id} className="flex flex-wrap items-start gap-5 rounded-lg border border-line bg-panel p-4">
              {u.croppedImageUrl && <ImageStrip src={u.croppedImageUrl} label={`Unknown #${i + 1} (cropped)`} />}
              {u.spectrum && (
      ...
      <footer className="border-t border-line-soft pt-4 text-xs text-t4">
        Generated by Spectro Web · {dateFmt.format(updatedAt)}
      </footer>
      </main>
    </ReportThemeShell>
  );
