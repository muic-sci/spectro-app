...
- **`store/transfer.ts`** — export/import a portable **`.spectro.json`** bundle (experiment JSON + base64 image blobs). Because data lives only in this browser, this lets a student back it up, hand it in, or move it between machines; import always remaps ids so a re-import never clobbers an existing experiment. Surfaced on the list page (per-experiment **Export**, top-level **Import**).
- **`store/export-csv.ts`** — builds the results CSV in the browser (was the `/api/.../export.csv` route) and triggers a download.
