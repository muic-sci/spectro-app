...
- **Guided wizard (in progress):** the laptop flow L0 → L1 → L2 is built.
  ...
  - `L3` `/experiments/[id]` — wizard **shell** (persistent frame: `StepRail` + `GuidancePanel` + per-step canvas + `WizardNav` Back/Continue). It loads the experiment + images/standards/unknowns and renders the canvas for `currentStep`; step navigation + the Continue gate are server actions (`web/src/app/experiments/[id]/actions.ts`). Built canvases: **L3.1 camera/ROI** (`RoiStep` — full-strip default or numeric rectangle; the visual drag-on-frame editor is still TODO), **L3.2 calibration** (`CalibrationStep` — capture lamp → peaks/slope/intercept/R² readout + `SpectrumChart` with peak markers + fit verdict), **L3.3 blank** (`BlankStep` — capture I₀, show its profile), and **L3.4 standards** (`StandardsStep` — add (concentration+unit+capture) / list (with A@λmax) / delete; gate ≥2). L3.5–L3.7 render a `ComingSoonStep` placeholder.
  - **Derived analysis** `web/src/lib/experiment-analysis.ts` (`deriveAnalysis`, pure): recomputes the in-memory science from stored profiles each render — per-standard absorbance spectra, a **single experiment λmax** (from the highest-concentration standard, or an override), each standard's A at that shared λmax, the Beer-Lambert curve (≥2 standards), and unknown concentrations. Pinned by `web/test/experiment-analysis.test.ts`. Prisma-`Json` parsers are centralised in `web/src/lib/experiment-json.ts` (parseRoi/parseCalibration/parseProfile). End-to-end verified: real pipeline (cal+blank+blue1+blue5 fixtures) → λmax≈582 nm, blue5 A≈1.316, curve R²≈1.0.
...
- **Not yet built:** L3.5–L3.7 canvases (absorbance review with λmax tap + Beer-Lambert curve chart, unknown, results + CSV export), the visual ROI drag editor, SSE realtime + the phone-side join/capture-upload route handlers (pairing is UI-only so far). `deriveAnalysis` already computes the curve + unknown concentrations — L3.5/L3.6/L3.7 mostly need their canvases (incl. an absorbance multi-line chart + a scatter/regression chart). No `Experiment.lambdaMax` field yet — λmax is auto-derived; add the field when wiring the interactive λmax adjust.
