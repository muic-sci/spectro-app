...
- Run a **CVE/SAST scan before committing** — `trivy fs --scanners vuln --severity HIGH,CRITICAL web` (and optionally `semgrep scan --config auto web/src`). **As of v3.7.2 the scan is clean (0 HIGH/CRITICAL).** The long-accepted findings (`postcss@8.4.31` + its `nanoid`, and a nested optional `sharp@0.34.x`) were `next`-internal pins with no fix available; `next@16.3.4` moved them to `postcss@8.5.23`/`nanoid@3.3.18`/`sharp@0.35.4` and cleared all of them. If `next`-internal pins go vulnerable again, prefer waiting for (or taking) the upstream `next` release — never npm `overrides`; and remember nothing in that layer ships, since the deployed artifact is nginx serving static `out/` (postcss runs at build time, sharp only in tests and `npm run analyze`).
