...
- **Data model:** `web/prisma/schema.prisma` — `User` (with `passwordHash`) + `PasswordResetToken` + a spectro domain that ports `project.dart`. The shared two-device unit is `Experiment`; it owns pairing/step state, `roi`, `calibration` (Json), `lightType` (`fluorescent`|`laser`) + `laserWavelengths` (Json `[r,g,b]`, laser mode only), the **global concentration `unit`** (one of `µM`/`mL`/`%`, chosen once at setup; `@default("µM")`), and `SpectralImage`/`Standard`/`Unknown` rows. **`Standard` has no `unit`** — the unit is experiment-global (the same unit applies to every standard + unknown), so it's stored once on `Experiment` and never re-entered per capture (migration `20260607000000_global_experiment_unit` adds `Experiment.unit`, backfills it from existing standards, drops `Standard.unit`). Allowed units + helpers (`CONCENTRATION_UNITS`/`DEFAULT_UNIT`/`normalizeUnit`) live in `experiment-meta.ts`. `SpectralImage.role` adds `laser` (the individual laser-line captures, tagged with `laserWavelength`) alongside `calibration`/`blank`/`standard`/`unknown`. **No Auth.js adapter tables** (Account/Session/VerificationToken) — sessions are stateless JWTs. **Migrations live in `web/prisma/migrations/`** (baseline `init`); prod runs `prisma migrate deploy`, dev runs `prisma migrate dev`.
...
- **Guided wizard (in progress):** the laptop flow L0 → L1 → L2 is built.
  ...
  - `L3` `/experiments/[id]` — wizard **shell** (persistent frame: `StepRail` + `GuidancePanel` + per-step canvas + `WizardNav` Back/Continue). It loads the experiment + images/standards/unknowns and renders the canvas for `currentStep`; step navigation + the Continue gate are server actions (`web/src/app/experiments/[id]/actions.ts`). **All 6 step canvases are built (L3.4 now merges the standards capture with the signal-review charts, so the graph builds live as standards are added):** **L3.1 camera/ROI** (`RoiStep` → `RoiBoxEditor` — captures the **lamp first** (you can't mark a region without seeing the strip); `RoiBoxEditor` (client) now owns the **horizontal/vertical orientation** toggle (`Experiment.orientation`) + **drag a box** over the image (mouse + touch, pointer events; box stored in image px) + "Use full strip" + a `<canvas>` "Region used for analysis" preview drawn from the loaded image (no server crop fetch). **Orientation is auto-detected from the ROI's colour gradient** (client-side, live as you draw/adjust the box): `scoreOrientation` (`analysis/image.ts`, pure) runs a one-way ANOVA on the ROI's RGB pixels — η²_horizontal = between-column variance / total, η²_vertical = between-row variance / total (subsampled to ~40k px so cost is image-size-independent). The larger η² is the suggested axis; its value (0..1) doubles as a **"Region clarity" goodness %** — how uniform the colour is along each line perpendicular to the dispersion axis (clean strip ≈ all colour variance along one axis, ~none across its width). Direction (red→blue vs blue→red) is irrelevant — calibration auto-flips. The chip **auto-follows** the detection while a box is adjusted (confident gate: goodness ≥ 0.35, margin ≥ 0.05) but **manual selection sticks** (clicking a chip stops auto-follow + commits; a "↺ Auto-detect" link re-enables); auto-changes persist only on **Save region** (like the box). Client glue: `analysis-client.suggestOrientation(url, roi)` (cached decode → `scoreOrientation`); pinned by `web/test/orientation.test.ts`. Changing the ROI *or* orientation re-extracts **in the browser** (`analysis-client.reextractAll`: decode every stored image, re-extract its profile + crop, recompute calibration) then POSTs to the Route Handler `POST …/reextract` (DB + crop storage only — no server-side sharp). This replaced the old server-side `reextractExperiment` for the web path and is what makes the toggle instant), **L3.2 calibration** (`CalibrationStep` — lamp → peaks/slope/intercept/R² + `SpectrumChart` w/ peak markers + fit verdict + `AlignedLampStrip`: the cropped lamp strip drawn under the chart with the detected peaks overlaid, so you can see each peak sits on a real emission line. It is a **client `<canvas>` component** that always renders a **horizontal band oriented blue (short λ) → red (long λ), left→right**: a **vertical** capture is rotated 90° to lie flat (CCW if λ ascends with pixel, CW if not), and a **negative-slope** (red→violet) capture is flipped, so blue always ends on the left. Peak dashes are positioned by **pixel position** (mirrored when the image is flipped), NOT by wavelength — so each dash sits on the *actual* emission line in the image (the calibration is a fit, so wavelength↔pixel isn't exact) and lines up with the chart above, which also plots peaks by pixel. A small nm axis labels the ends (blue ← left, red → right). To keep the chart aligned, `SpectrumChart` takes a `reverseX` prop (XAxis `reversed`) that the calibration step/report set when `slope < 0`, and the strip is embedded **inside the chart card** (`bare` prop) directly under the pixel axis with matching plot-area padding (PAD_LEFT 50 = YAxis width 46 + left margin 4, PAD_RIGHT 18 = right margin) so the dashes line up vertically). The chart+strip card is the shared **`SpectrumWithStrip`** component (profile chart with colour-coded calibration wavelengths + `reverseX` flip + embedded cropped strip), reused by the blank step, **L3.3 blank** (`BlankStep` — the I₀ profile shown via `SpectrumWithStrip` too: same calibration-style view with the blue→red spectrum strip under the pixel axis and the auto-flip, so I₀ reads consistently with the lamp; falls back to a plain chart if no calibration yet), **L3.4 standards + signal review (merged)** (`StandardsStep` — add (concentration+unit+capture) / list (with A@λmax) / delete, **plus the live charts that used to be a separate `absorbanceReview` step**: as soon as the first standard yields a spectrum it renders the `AbsorbanceChart` overlay + λmax marker + `LambdaMaxControl` (`setLambdaMaxAction`), with **each standard's cropped strip embedded under the spectra chart** (one `AlignedLampStrip` per standard — blue→red, calibration peaks overlaid, labelled by concentration in the matching series colour; same plot-area alignment as the calibration/blank `SpectrumWithStrip`, fed the lamp profile's pixel domain via the page's `stripDomain`). The chart + strips are wrapped in the client **`SignalSpectraCard`** (`web/src/components/wizard/signal-spectra-card.tsx`), which **owns the optimistic λmax** so the chart marker and every strip line move **together, live, while the λmax line is being dragged** (not just on commit). The numeric **`LambdaMaxControl` is merged into this card** — it renders **`bare`** (no card chrome/help text) in the card header beside the title, so there's one card not two. **There is no "Set" button** — the number input commits on **blur/Enter** (imperative `setLambdaMaxAction`, skipping invalid/unchanged values). The **Auto** button reverts to the auto-derived λmax and is **colour-coded to mirror the λmax line**: amber (`var(--warn)`) when λmax is auto, greyed (`var(--t4)`) once it's a manual override (it takes `isManual`; no-ops when already auto). **In both modes the strips show a single (non-interactive) λmax line instead of the R/G/B calibration peaks** (`AlignedLampStrip` takes `lambdaMax`/`lambdaMaxColor`; standard strips render slim — `bandHeight={32}`, `showWavelengthAxis={false}` — with the concentration+colour-dot in the left gutter via `leadingLabel`) — that line is static *as a control* but tracks the shared optimistic value during a drag. **The λmax line is dragged on the graph itself**: `AbsorbanceChart` is now **controlled** (renders the `lambdaMax` prop; no internal commit) and takes `onLambdaDrag`/`onLambdaCommit` callbacks — when present, a transparent grab handle (a `role="slider"` div with a small grip) overlays the Recharts `ReferenceLine`, positioned with the same plot-area geometry the strips use (`PLOT_LEFT = 50` = YAxis 46 + left margin 4, `PLOT_RIGHT_PAD = 18` = right margin) so it lands on the line and aligns with the strips below. Dragging (pointer events → mouse + touch; pointer x → wavelength linearly across the `[dataMin,dataMax]` domain) fires `onLambdaDrag` continuously → the card's `setLive` → re-renders chart + strips; on release `onLambdaCommit` → the card commits a manual λmax via `setLambdaMaxAction` + `router.refresh()`, holding the optimistic value until the committed prop catches up (cleared during render). Dragging is enabled in **both** modes (the chart is mode-agnostic; matches the original "tap the spectrum chart to set λmax"). **The λmax marker colour encodes auto vs manual** (both the chart line and the strip line): `var(--warn)` amber while auto-derived, `var(--accent-color)` cyan once the user pins it (override `Experiment.lambdaMax != null`, passed to `StandardsStep` as `lambdaMaxOverride`); `LambdaMaxControl`'s **Auto** button reverts to amber. Once ≥2 standards are measurable the `CalibrationCurveChart` scatter+fit + R²/slope Readouts appear too. **Gate is now `derived.curve`** (≥2 *usable* standards, not just ≥2 rows). The `absorbanceReview` `WorkflowStep` enum value is **retired from the rail** (`WORKFLOW_STEPS` drops it) but kept in the Prisma enum for legacy rows — the wizard page **redirects `currentStep === "absorbanceReview"` → `standards`**, same as it maps `experimentSetup` → `cameraRoiSetup`. The old `absorbance-review-step.tsx` was deleted; `AbsorbanceReviewStep` no longer exists.), **L3.5 unknown** (`UnknownStep` — capture → A@λmax + concentration Readout + out-of-range warn + unknown plotted on the curve; **supports many unknowns** — each capture is its own `Unknown` row and the upload control takes `multiple` so several photos can be picked at once, each analysed + uploaded in turn as a separate unknown. Unknowns are ordered `createdAt asc` in the wizard, report and CSV so the `#1/#2…` numbering is stable + consistent across all three — `deriveAnalysis` keeps the input order for unknowns, unlike standards which it sorts by concentration), **L3.6 results** (`ResultsStep` — summary Readouts + data table + **CSV export**, and links to the **full report**). `ComingSoonStep` remains only as a defensive default.
  - **Full report** `/experiments/[id]/report` (standalone, print-friendly page): assembles the captures + **every plot** + all Readouts + the data table, built on the **same components as the wizard** so it looks consistent — the calibration & blank sections use `SpectrumWithStrip` (blue→red strip under the axis, `reverseX` flip, colour-coded peaks) and the calibration section adds the shared `DetectedPeaksTable` (wavelength/pixel/intensity/fit-λ); the lamp still shows the ROI box via `RoiPreview`; unknown absorbance plots tint the λmax marker by wavelength. The **standards section is the same `SignalSpectraCard` as the wizard, rendered `readOnly`** (no λmax control/header, non-draggable λmax line) — i.e. the "{signal} spectra" chart + slim per-standard strips with the λmax line, **not** an editable view (the report never changes λmax); the Beer-Lambert/Calibration curve follows it. The report is **self-explanatory for students**: a top **"Method"** section walks the whole pixel→concentration pipeline step-by-step (lock focus/exposure → ROI → average perpendicular to dispersion → sRGB gamma-linearise → luminance `0.299R+0.587G+0.114B` (lamp uses `max(R,G,B)`) → pixel→λ fit → blank I₀/background → `A=−log₁₀(I/I₀)` or `F=I−I₀` → λmax → linear curve `{t.law}` → `c=(signal−intercept)/slope`), and **every section opens with a mode-aware `Explainer` box** stating what its plots show + the maths (`Explainer`/`Formula` helpers in `report/page.tsx`). `PrintButton` → `window.print()` (Save as PDF); `@media print` in globals.css hides `.no-print` chrome, prints on a **white** background, and forces colour. **The report has its own light/dark toggle** (the app is otherwise dark-only): `ReportThemeShell`/`ReportThemeToggle` (`web/src/components/report/report-theme.tsx`) scope a theme to the report subtree via `data-theme` on a wrapper — a **light OKLCH palette** is defined under `[data-theme="light"]` in globals.css and re-themes the whole subtree, **including the Recharts charts**, because the Tailwind tokens are `@theme inline` (utilities reference `var(--token)` directly). The choice persists in `localStorage` (via `useSyncExternalStore`, SSR-safe), and **printing is always forced to light** (the shell flips the wrapper's `data-theme` to `light` on `beforeprint`, restores on `afterprint`) so output is ink-light on white regardless of the on-screen theme. Reuses the chart components + `deriveAnalysis`.
  - **Derived analysis** `web/src/lib/experiment-analysis.ts` (`deriveAnalysis`, pure): recomputes the in-memory science from stored profiles each render — **mode-aware** (absorbance or fluorescence — see Key Domain Concepts): per-standard signal spectra, a **single experiment λmax** (`Experiment.lambdaMax` override, else from the highest-concentration standard), each standard's signal at that shared λmax, the calibration curve (≥2 standards), and unknown concentrations (`determineConcentration`, with an `outOfRange` flag). Takes the experiment-global `unit` and echoes it as `DerivedAnalysis.unit` (all readers — wizard/report/CSV — use that, not a per-standard unit). Pinned by `web/test/experiment-analysis.test.ts`. Prisma-`Json` parsers centralised in `web/src/lib/experiment-json.ts`.
